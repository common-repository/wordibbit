...
class Spyc
{
    ...
    */
    ...
    {
        if (is_array($value)) {
            if (empty($value)) return $this->_dumpNode($key, array(), $indent, $previous_key);
    ...
    private function _dumpNode($key, $value, $indent, $previous_key = - 1)
    {
        ...
        if (strpos($value, "\n") !== false || strpos($value, ": ") !== false || strpos($value, "- ") !== false || strpos($value, "#") !== false || strpos($value, "<") !== false || strpos($value, ">") !== false || strpos($value, "[") !== false || strpos($value, "]") !== false || strpos($value, "{") !== false || strpos($value, "}") !== false) {
    ...
    private function _doFolding($value, $indent)
    {
        ...
        if ($this->_dumpWordWrap !== 0 && strlen($value) > $this->_dumpWordWrap) {
            ...
            if ($this->setting_dump_force_quotes && is_string($value)) $value = '"' . $value . '"';
        }
        return $value;
    }
    // LOADING FUNCTIONS
    ...
    {
        if (!empty($input) && strpos($input, "\n") === false && file_exists($input)) return file($input);
    ...
    private function _toType($value)
    {
        ...
        while (0);
        if ($is_quoted) return strtr(substr($value, 1, -1), array('\\"' => '"', '\'\'' => '\'', '\\\'' => '\''));
        if (strpos($value, ' #') !== false) $value = preg_replace('/\s+#(.+)$/', '', $value);
    ...
    {
        if (count($array) > 1) return $this->addArrayInline($array, $indent);
        $key = key($array);
    ...
    function revertLiteralPlaceHolder($lineArray, $literalBlock)
    {
        foreach($lineArray as $k => $_) {
            ...
            else if (substr($_, -1 * strlen($this->LiteralPlaceHolder)) == $this->LiteralPlaceHolder) $lineArray[$k] = rtrim($literalBlock, " \r\n");
        }
        return $lineArray;
    }
